...
%}
    properties
        filename_seriou_suspected; % 用于整体扫描结果显示框：'文件名_严重故障点百分比_疑似故障点百分比'
        serioufilename_seriou_suspected_fault_points; % 用于严重故障-数据文件显示框：'文件名_严重故障点个数_疑似故障点个数'
        filen__scanning_suspected_fault_points; % 用于疑似故障-数据文件显示框：'文件名_严重故障点个数_疑似故障点个数'
        num_normal_files; % 正常文件的个数
        num_seriou_files; % 严重故障文件的个数
        num_suspected_files; % 疑似故障文件的个数
    end

    methods
        function obj = TDMS3(set_vibration_threshold,set_scanning_sensitivity,filename,filepath)

            % 显示扫描
            h = waitbar(0.01,'正在全域扫描诊断','CreateCancelBtn','setappdata(gcbf,''canceling'',1)');
