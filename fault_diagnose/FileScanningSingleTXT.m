classdef FileScanningSingleTXT
    % 用于扫描单个 TXT 文件
    ...
    methods
        function obj = FileScanningSingleTXT(scn_mode,scn_channel,scn_channel_max,scn_serious_threshold,scn_suspected_threshold,set_scanning_sensitivity,filename,filepath) % 类的构造函数，用于初始化类的属性
            % 读入单个txt文件的数据
            if filepath(end) ~='\'
                filepath = [filepath '\'];
            end
            [channelData channelNames] = TXT_scn_readChannelOrGroup(filepath,filename);
...
%             channelData_scn_suspected(:) = {0};
            ...
            for i = scn_channel
                ...
                %严重
                % channelData_scn这一行的数据统一除（str2num(scn_channel_max(j))*str2num(scn_serious_threshold(j))）取商
