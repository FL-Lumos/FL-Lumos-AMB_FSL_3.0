...
function [channelData channelNames] = TDMS_readChannelOrGroup(fileName)

%---------------从配置文件中读取通道信息-----------------%
    % 从配置文件中读取对于txt文件数据通道的定义：
    % 获取当前运行的 .m 文件的路径
    currentPath = fileparts(mfilename('fullpath'));    
    % 指定相对路径的文件夹
    relativeFolderPath = '..\config\tdmsInfo_config.mat';    
    % 构建完整的文件夹路径
    configFilePath = fullfile(currentPath, relativeFolderPath);
    if exist(configFilePath, 'file') == 2
        % 读取配置文件
        loadedData = load(configFilePath);        
        % 获取通道定义
        TXT_channeNames = loadedData.tdmsInfo.TXT_channeNames;        
        % 现在 channelNames 变量包含了从配置文件中读取的通道定义
        disp('成功读取配置文件中的通道定义。');
    else
        disp('配置文件不存在。');
    end
